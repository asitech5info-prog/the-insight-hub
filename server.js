...
// API: Add review (with email support)
app.post('/api/review', async (req, res) => {
  try {
    const { blogId, name, email, rating, comment } = req.body;
    const newReview = {
      id: uuidv4(),
      blogId,
      name,
...
app.get('/api/reviews/:blogId', async (req, res) => {
  try {
    const collection = await col('reviews');
    const reviews = await collection.find({ blogId: req.params.blogId }).sort({ _id: -1 }).toArray();
    res.json(reviews);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
